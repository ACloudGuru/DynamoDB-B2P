...
##          IDBucket cloud formation template applied, bucket details available
...
def s3_upload(BucketName, Sourcefile): # given a file, upload and return url
    ...
    #return s3_c.generate_presigned_url('get_object', {'Bucket' : BucketName, 'Key' : Sourcefile}, ExpiresIn=7776000)
...
if __name__ == "__main__":
    ...
    while True:
        if py3:
            s3bucket = input('Enter the name of the S3 bucket created by IDBucket.cfn: ')
        else:
            s3bucket = raw_input('Enter the name of the S3 bucket created by IDBucket.cfn: ')
