...
# A Cloud Guru - Data Generating Script - Section 3 - Data Model 2
...
##          IDBucket.cfn cloud formation template applied, bucket details available
...
def d_table(): # define table configuration
    ...
    table_config['lo_students']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 100 }
    table_config['lo_teachers']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 100 }
    table_config['lo_modules']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    table_config['lo_courses']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    table_config['lo_courseinstances']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    table_config['lo_attendance']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    table_config['lo_exams']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 100 }
    table_config['lo_bookings']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    table_config['lo_counters']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 100, 'WriteCapacityUnits' : 100}
    table_config['lo_studentgroups']['ProvisionedThroughput'] = { 'ReadCapacityUnits' : 2, 'WriteCapacityUnits' : 10 }
    ...
    table_config['lo_studentgroups']['AttributeDefinitions'] = [\
        {'AttributeName' : 'g_id', 'AttributeType' : 'N'}, {'AttributeName' : 'ci_id', 'AttributeType' : 'N'}]
...
def p_table (Table, idbucket): # Populate Tables
    numofrows = ( 2500 if Table=='lo_students' \
                ...
                else 1000 if Table=='lo_courseinstances' \
        ...
        with db_r.Table(Table).batch_writer() as batch:
            for c in tqdm.tqdm(range(1, numofrows+1)):
                if Table=='lo_students':
                    ...
                    batch.put_item(Item=item_gen(Type="student", Counter=counter, Idbucket=idbucket))
                else:
                    counter = get_counter(keyname='t_id')
                    batch.put_item(Item=item_gen(Type="teacher", Counter=counter, Idbucket=idbucket))

    if Table == "lo_exams":
        with db_r.Table(Table).batch_writer() as batch:
            for c in tqdm.tqdm(range(1,numofrows+1)):
                counter = get_counter(keyname='e_id')
                batch.put_item(Item=item_gen(Type="exam", Counter=counter, Idbucket=idbucket))

    if Table == "lo_modules":
        with db_r.Table(Table).batch_writer() as batch:
            for c in tqdm.tqdm(range(1,numofrows+1)):
                counter = get_counter(keyname='m_id')
                batch.put_item(Item=item_gen(Type="module", Counter=counter, Idbucket=idbucket))
    if Table == "lo_courses":
        with db_r.Table(Table).batch_writer() as batch:
            for c in tqdm.tqdm(range(1, numofrows+1)):
                counter = get_counter(keyname='c_id')
                batch.put_item(Item=item_gen(Type="course", Counter=counter, Idbucket=idbucket))
...
if __name__ == "__main__":
    ...
    for x in tables_to_create:
                ...
                p_table(Table=x, idbucket=s3bucket)
